"""Migration: add 'passed' to jobs status CHECK constraint.

SQLite does not support ALTER CONSTRAINT, so this rebuilds the jobs table
with the updated CHECK. Safe to re-run (guards on existing constraint).

Usage:
    python -m db.migrate_002_add_passed_status
"""
...
def migrate():
    ...
    if ddl and "'passed'" in ddl[0]:
        print("Migration 002: already applied (passed status exists).")
        return

    print("Migration 002: adding 'passed' to jobs status constraint...")
