...
| status                | TEXT NOT NULL DEFAULT 'new' | CHECK: new\|passed\|reviewed\|applied\|rejected\|offer |
...
The default index view uses `exclude_status='passed'` to hide passed jobs.

## Status Workflow

```
new → passed     (user chose not to apply during review)
new → reviewed   (user is interested, planning to apply)
reviewed → applied   (application submitted)
